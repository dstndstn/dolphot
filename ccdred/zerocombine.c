...
double *dmin,*dmax,*noise,*exptime,c2=0,nsig,PGAIN,nrej[4]={0,0,0,0};
float PDMIN,PDMAX;
int N,min,avg;
imtype *data;
ftype *fits,cfits;
...
void cleanchip(void) {
   ...
   if (first) {
      list=(double2*)calloc(sizeof(double2),N);
      if (!list) merr();
      first=0;
   }
   for (z=0;z<cfits.img.Z;z++) for (y=0;y<cfits.img.Y;y++) for (x=0;x<cfits.img.X;x++) {
	 ...
	 for (i=0;i<n;i++) {
	    ...
	    dmid=list[i][1]/PGAIN*nsig;
	    if (mid>0) dmid=sqrt(dmid*dmid+nsig*nsig*mid/PGAIN+c2*c2*mid*mid);
...
int main(int argc,char**argv) {
   ...
   if (argc<4) {
      printf("Usage: %s <fits files> <sigma clip> <use min>\n",*argv);
      printf("Recommend nsig=3, use min=0\n");
      return 1;
   }
   Ntot=argc-3;
   ...
   for (ext=0;ext<=cfits.Next;ext++) {
      ...
      for (i=0,N=0;i<Ntot;i++) if (!strcmp(btype[i],"zero")) {
	 double IEXP,IGAIN,IRN,IEPOCH,IAIR,IEXP0;
	 float IDMAX,IDMIN;

	 if (!ext) {
	    if (N==0) printf("Generating zero frame\n");
	 ...
	 N++;
      ...
      if (!ext) {
	 sprintf(str,"%s.fits",zerobase);
	 fout=writefitsh(str,&cfits,1);
      }
      else writeexth(fout,&(cfits.img),0);
      writeimage(fout,&(cfits.img));
      for (i=0;i<Ntot;i++) if (!strcmp(btype[i],"zero")) {
	 if (fits[i].img.Nmax) free(fits[i].img.cards);
	 fits[i].img.Nmax=0;
   ...
   for (i=0;i<Ntot;i++) if (!strcmp(btype[i],"zero")) fclose(fin[i]);
   fclose(fout);
