...
void interpAndersonPSF(int x,int y)
{
   ...
   */
   int xx,yy;
   for (xx=0;xx<101;xx++) for (yy=0;yy<101;yy++) apsf[yy][xx]=fits.img.data[0][yy][xx];
}
...
void mergepsf(char*filt)
{
   ...
   sprintf(fn,"psf_wfc3ir_%s.fits",filt);
   for (ptr=fn;*ptr;ptr++) if (*ptr>='A' && *ptr<='Z') (*ptr) += ('a'-'A');
   readfits(fn,&fits,0);
   if (fits.Next!=0 || fits.img.X!=101 || fits.img.Y!=101 || fits.img.Z!=1) {
   ...
   for (y0=0;y0<wfc3_nypsfpos[cm];y0++) for (x0=0;x0<wfc3_nxpsfpos[cm];x0++) {
      interpAndersonPSF(64+128*x0,64+128*y0);
